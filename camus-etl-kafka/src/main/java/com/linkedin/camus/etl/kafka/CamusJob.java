...
public class CamusJob extends Configured implements Tool {
    ...
    public void run() throws Exception {
        ...
        FileSystem fs = FileSystem.get(job.getConfiguration());
        System.out.println("FileSystem: " + fs.getHomeDirectory());
        ...
        //sort execution by modification date
        List<FileStatus> executionList = Arrays.asList(executions);
        Collections.sort(executionList, new Comparator<FileStatus>() {
            public int compare(FileStatus o1, FileStatus o2) {
                Date date1 = new Date(o1.getModificationTime());
                Date date2 = new Date(o2.getModificationTime());
        ...
        for (int i = 0; i < executionList.size() && limit < currentCount; i++) {
            FileStatus stat = executionList.get(i);
        ...
        if (executionList.size() > 0) {
            ...
            Path previous = executionList.get(executionList.size()-1).getPath();
        ...
        String timeParam = new DateTime().toString(dateFmt);
        Path newExecutionOutput = new Path(execBasePath, timeParam);
        Path newCompleteOutput = new Path(execHistory, timeParam);
        ...
        System.out.println("InputFormatClass initialized");
        job.setOutputFormatClass(EtlMultiOutputFormat.class);
        System.out.println("OutputFormatClass initialized");
        job.setOutputKeyClass(EtlKey.class);
        System.out.println("OutputKeyClass initialized");
        job.setOutputValueClass(AvroWrapper.class);
        System.out.println("OutputValueClass initialized");

        job.setNumReduceTasks(0);

        stopTiming("pre-setup");
        System.out.println("pre-setup done. submitting job");
        job.submit();
        System.out.println("job submitted. waiting for completion");
        job.waitForCompletion(true);
        
        System.out.println("dumping all counetrs");
        // dump all counters
        ...
        stopTiming("hadoop");
        System.out.println("all counetrs dumped");
        ...
        System.out.println("starting to record errors");
        for (FileStatus f : fs.listStatus(newExecutionOutput, new PrefixFilter(
        ...
        System.out.println("errors recorded");
        ...
        System.out.println("Renaming " + newExecutionOutput + " to " + newCompleteOutput);
    ...
    public int run(String[] args) throws Exception {
        ...
        System.out.println("**");
        System.out.println(props.getProperty("etl.destination.path"));
